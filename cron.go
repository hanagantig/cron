...
type Entry struct {
	...
	// JobKey is a unique key for job, which may be used to set a distributed lock
	JobKey string
...
func (c *Cron) AddFunc(spec, jobKey string, cmd func()) (EntryID, error) {
	return c.AddJob(spec, jobKey, FuncJob(cmd))
}

// AddJob adds a Job to the Cron to be run on the given schedule.
// The spec is parsed using the time zone of this Cron instance as the default.
// An opaque ID is returned that can be used to later remove it.
func (c *Cron) AddJob(spec, jobKey string, cmd Job) (EntryID, error) {
	schedule, err := c.parser.Parse(spec)
	if err != nil {
		return 0, err
	}
	return c.Schedule(schedule, cmd, jobKey), nil
}

// Schedule adds a Job to the Cron to be run on the given schedule.
// The job is wrapped with the configured Chain.
func (c *Cron) Schedule(schedule Schedule, cmd Job, jobKey string) EntryID {
	c.runningMu.Lock()
		...
		for {
			...
			case now = <-timer.C:
				...
				for _, e := range c.entries {
					...
					c.startJob(e.WrappedJob, e.JobKey)
					e.Prev = e.Next
